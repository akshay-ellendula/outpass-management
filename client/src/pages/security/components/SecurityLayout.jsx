      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 space-y-1">
          <Link 
            ...
            className={`w-full flex items-center gap-3 px-6 py-3 transition ${isActive('/security/dashboard')}`}
          >
          ...
          <Link 
            to="/security/logs" 
            onClick={handleLinkClick}
            className={`w-full flex items-center gap-3 px-6 py-3 transition ${isActive('/security/logs')}`}
          >
          ...
          <Link 
            to="/security/profile" 
            onClick={handleLinkClick}
            className={`w-full flex items-center gap-3 px-6 py-3 transition ${isActive('/security/profile')}`}
          >
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center gap-3 mb-4 px-2">
            <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-xs border border-blue-200">
              {user?.name ? user.name.substring(0,2).toUpperCase() : 'GD'}
            </div>
            <div className="overflow-hidden">
              <p className="text-sm font-bold text-slate-900 truncate">{user?.name || 'Guard'}</p>
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2 truncate">
              <span className="bg-red-50 text-red-600 p-1 rounded shrink-0">
